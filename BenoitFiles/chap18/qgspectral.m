...
% PARAMETERS
% M and N must be a multiply of 4
M=128;  % 16 64 128 256 512
N=M;
DX=5000;  % 10000
LX=2*pi;  % M*DX
LY=LX;
AH=0; % 10 0 0.000001/4
BH=3.125*10^(-9)/4;  % BH=3.125*10^(-7)/4;
dissrate=0.000000;
dt=0.05;
...
for ic=1:M
    for jc=1:N
        ...
        psifft(ic,jc)=(randn(1,1))*kk/(1+kk^4/10^4)+i*(randn(1,1))*kk/(1+kk^4/10^4);
...
for n=0:NT
    ...
    if istepping==4 
        ...
        rh1=-dissrate*omfft-jacobfft(psifft,omfft,LX,LY)+AH*laplacianfft(omfft,LX,LY)-BH*bilaplacianfft(omfft,LX,LY);
        om1=omfft+dt/2*rh1;
        psifft=inversepoissonfft(om1,LX,LY);
        rh2=-dissrate*om1-jacobfft(psifft,om1,LX,LY)+AH*laplacianfft(om1,LX,LY)-BH*bilaplacianfft(om1,LX,LY);
        om1=omfft+dt/2*rh2;
        psifft=inversepoissonfft(om1,LX,LY);
        rh3=-dissrate*om1-jacobfft(psifft,om1,LX,LY)+AH*laplacianfft(om1,LX,LY)-BH*bilaplacianfft(om1,LX,LY);
        om1=omfft+dt*rh3;
        psifft=inversepoissonfft(om1,LX,LY);
        rh4=-dissrate*om1-jacobfft(psifft,om1,LX,LY)+AH*laplacianfft(om1,LX,LY)-BH*bilaplacianfft(om1,LX,LY);
        omfft=omfft+dt/6*(rh1+2*rh2+2*rh3+rh4);
    end
    if istepping==2
        if n==0
            omfftold=omfft;
            psifft=inversepoissonfft(omfft,LX,LY);
            rh1=-dissrate*omfft-jacobfft(psifft,omfft,LX,LY)+AH*laplacianfft(omfftold,LX,LY)-BH*bilaplacianfft(omfftold,LX,LY);
            omfft=omfft+dt*rh1;
        else
            psifft=inversepoissonfft(omfft,LX,LY);
            rh1=-dissrate*omfftold-jacobfft(psifft,omfft,LX,LY)+AH*laplacianfft(omfftold,LX,LY)-BH*bilaplacianfft(omfftold,LX,LY);
            rh1=omfftold+2*dt*rh1;
            omfftold=omfft;
            omfft=rh1;
        end
    end
    [kene(n+1),kens(n+1),enet(n+1),enst(n+1)]=kmean(omfft,psifft,LX,LY);
    ...
    if mod(n,60)==0
       % title(num2str((t)));
        ...
        % colorbar('horiz')
        if mod(n,10)==0
            aaaa= [ 'QGspectral' num2str(10000+n) '.png']
            print('-dpng', aaaa)
        end
        pause(0.01)
    end
